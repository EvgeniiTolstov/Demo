...
export async function GET({ url, params, cookies, request }) {
    ...
    if (valueMin !== '') {
        props[keyMin] = {
            [db.Sequelize.Op.gte]: valueMin
        }
    }
    if (valueMax !== '') {
        props[keyMax] = {
            [db.Sequelize.Op.lte]: Number(valueMax)
        }
    }
    ...
    const props2 = await [...url.searchParams].reduce((acc, [key, value]) => {
        if (key === 'limit' || key === 'offset' || key === 'order' || key === 'order_by' || key === 'summ' || value === '') {
            return acc
        }
    ...
    if (valueSearch !== '') {
        props[keySearch] = {
            [db.Sequelize.Op.like]: `%${valueSearch}%`
        }
    }
