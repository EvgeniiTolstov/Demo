<script>
  ...
  import {Countcart, SortOrder, SortLimit, Countwishlist} from '$lib/stores.js';
...
let loaderpage = false;
    $: totalItems  = Number(data.products.product.data.count)
    let limit = Number($SortLimit)
    let sort = $SortOrder
    $: offset = Number(data.products.product.meta.offset)?Number(data.products.product.meta.offset):0;
    $: console.log(data.products.product.meta.offset)
...
function pageSelects() {
  ...
  $: if (data) {
    ...
    <Loader />
  {/if}
   <meta name="description" content="Каталог запчастей для отечественных грузовых автомобилей оптом" />
    <meta name="keywords" content="Каталог запчастей для отечественных грузовых автомобилей оптом" />
    <title>Каталог запчастей для отечественных грузовых автомобилей оптом</title>
    
   <div class="">
    {#if addcaetalert || wishlistAlert}  
              
  <div id="popup-modal" tabindex="-1" class="fixed top-0 left-0 right-0 z-50  w-full p-4 overflow-x-hidden overflow-y-auto  max-h-full">
      <div class="relative p-4 w-full max-w-md max-h-full " >
          <div class="relative bg-white rounded-lg shadow dark:bg-green-400">
              ...
              <div class="p-4 md:p-5 text-center">
                  <svg class="mx-auto mb-4 text-gray-800 w-12 h-12 dark:text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
                      <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 11V6m0 8h.01M19 10a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/>
                  </svg>
                  {#if addcaetalert}
                  <h3 class="mb-5 text-lg font-normal text-gray-900 dark:text-gray-600">Товар добавлен в корзину</h3>
                  {:else}
                  <h3 class="mb-5 text-lg font-normal text-gray-900 dark:text-gray-600">Товар добавлен в список желаний</h3>
                  {/if}
     ...
     <section class="py-10 bg-gray-100">
       <div class="container mx-auto px-4">
         ...
         <ul class="flex flex-wrap mb-8 items-center">
           ...
           <li><a class="inline-block text-xs font-bold font-heading uppercase text-gray-300" > Каталог</a></li>
         </ul>
   ...
   Запчасти для отечественных грузовых автомобилей  
           ...
           <div class="w-full lg:w-auto px-4 flex flex-wrap items-center">
             <div class="w-full sm:w-auto mb-4 sm:mb-0 mr-5">
               <select bind:value={sort} on:change={sortSelects} class="pl-8 py-4 bg-white text-lg border border-gray-200 focus:ring-blue-300 focus:border-blue-300 rounded-md" name="" id="">
                 <option value="order_by=id&order=ASC" >Сортировать по порядку</option>
                 <option value="order_by=price&order=ASC">Сортировать по увеличению цены.</option>
                 <option value="order_by=price&order=DESC">Сортировать по уменьшению цены.</option>
               </select>
             </div>

             <div class="w-full sm:w-auto mb-4 sm:mb-0 mr-5">
              <select bind:value={limit} on:change={limitSelects} class="pl-8 py-4 bg-white text-lg border border-gray-200 focus:ring-blue-300 focus:border-blue-300 rounded-md" >
             ...
             <div class="flex flex-wrap -mx-2">
               ...
               <div class="w-1/2 md:w-1/3 px-2 mb-4">
                <div class="py-6 px-2 text-center bg-gray-50">
                  <a class="font-bold font-heading" data-sveltekit-preload-data="tap" href="/catalog/{category.slug}" on:click={pageSelects} >{category.title}</a>
                </div>
           ...
           <div class="hidden lg:block w-1/4 px-3">
             <div class="mb-6 py-10 px-12 bg-gray-50">
               ...
               <ul>
                ...
                <li class="mb-4" ><a class="text-lg" href="/catalog/{category.slug}" on:click={pageSelects} ><span>{category.title}</span></a></li>
             ...
             <div class="flex flex-wrap -mx-3">
               ...
               <div class="w-full sm:w-1/2 md:w-1/3 px-3 mb-8">
                 <div class="p-6 bg-gray-50">
                   ...
                   <button on:click={() => addWishlist(product.id)} class="ml-auto mr-2 flex items-center justify-center w-12 h-12 border rounded-lg hover:border-gray-500" href="#">
                   ...
                   <button on:click={() => addToCart(product.id)} class="mr-2 flex items-center justify-center w-12 h-12 border rounded-lg hover:border-gray-500" href="#">
